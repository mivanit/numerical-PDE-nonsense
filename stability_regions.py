...
def exprprint(
  ...
  if not len(expr) == len(rhs):
    raise ValueError(f"list of `expr`'s and `rhs`'s should be equal length, got {len(expr)=} {len(rhs)=}")
...
def analyze_diffop_matrix(
  ...
  for key,matfunc in MATRIX_DIFFOPS.items():
    ...
    print(
      f" - difference operator ${key}$, for $N={N}$, with {bdry} boundary conditions:\n",
      "  $$ ",
      key,
      " = ",
    ...
    eigvals : List[Expr] = list(sym.simplify(e) for e in mat.eigenvals().keys())
    print(
      "    $$ _{",
      latex(Matrix(eigvals)),
      "} $$",
    )
